...
public class RequestStandBlockEntityRenderer implements BlockEntityRenderer<RequestStandBlockEntity> {
    ...
    public RequestStandBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
        itemRenderer = context.getItemRenderer();
    }

    @Override
    public void render(RequestStandBlockEntity entity, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d cameraPos) {
        int lightAbove = WorldRenderer.getLightmapCoordinates(entity.getWorld(), entity.getPos().up());
        World world = entity.getWorld();

        if (entity.getInputCount() > 0) {
            ItemStack product = entity.getStack(0);
            // Product
            int productCount = 1;
        ...
        if (entity.getInputBlockCount() > 0) {
            ...
            ItemStack emeraldStack = entity.getStack(3);
            matrices.push();
